...
* **C.2 Notícias e benefícios (torcedor):** `GET /api/news`, `GET /api/news/{id}`, `GET /api/benefits/eligible`, `GET /api/benefits/offers/{offerId}` (respostas incluem `bannerUrl`, oferta camisa: `isShirtCustomizationOffer`, `shirtSizes`, `shirtModels`, `redemptionWorkflowStatus`), `POST /api/benefits/offers/{offerId}/redeem` (JWT; corpo opcional com campos de camisa **e** endereço de entrega `deliveryCep`, `deliveryNeighborhood`, `deliveryStreet`, `deliveryNumber`, `deliveryCity`, `deliveryState` para ofertas de camisa; frontend consulta **ViaCEP** para autopreencher endereço; `400` com `validation_failed` quando inválido); `ITorcedorNewsReadPort`, `ITorcedorBenefitsReadPort`, `ITorcedorBenefitRedemptionPort`; CQRS em `Modules/Torcedor`; SPA `/news`, `/news/:newsId`, `/benefits`, `/benefits/:offerId`, carrossel de benefícios na home (`DashboardPage`, layout `bannerUrl`: proporção `300:148` + texto reduzido); ver `docs/architecture/parte-c2-noticias-beneficios-torcedor.md`.
