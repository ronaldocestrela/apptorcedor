...
* **Frontend:** `frontend/` — React + Vite, login, convite staff (`/accept-staff-invite`), armazenamento de tokens, interceptor Axios com refresh, rotas protegidas, **`/api/auth/me` com `permissions`** e painel **`/admin`** (dashboard, staff, diagnóstico, configurações, auditoria, role × permissão editável, membership por ID); ver `docs/frontend/backoffice.md`.
