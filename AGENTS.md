...
* **B.7 Digital Card (admin):** tabela `DigitalCards`, API `GET /api/admin/digital-cards`, `GET .../issue-candidates`, `GET .../{id}`, `POST .../issue`, `POST .../{id}/regenerate`, `POST .../{id}/invalidate`; permissões `Carteirinha.Visualizar`, `Carteirinha.Gerenciar`; template de exibição fixo em código na API; SPA `/admin/digital-cards` (emissão por lista de associações Ativas sem carteirinha ativa); ver `docs/architecture/parte-b7-digital-card-admin.md`.
* **B.8 Games & Tickets (admin):** tabelas `Games`, `OpponentLogoAssets`, `Tickets` (incl. `RequestStatus` de solicitação: `Pending` / `Issued`); campo `Games.OpponentLogoUrl`; API `GET/POST /api/admin/games`, `GET/PUT/DELETE /api/admin/games/{id}` (DELETE desativa), `GET/POST /api/admin/games/opponent-logos` (biblioteca + upload de logo do adversário; política `GamesOpponentLogosUpload` = `Jogos.Criar` **ou** `Jogos.Editar`), `GET /api/admin/tickets` (filtro `requestStatus` + resposta com `requestStatus` e `membershipPlanName`), `GET .../{id}`, `POST .../reserve`, `POST .../{id}/purchase`, `.../sync`, `.../redeem`, `PATCH .../{id}/request-status`; permissões `Jogos.Visualizar`, `Jogos.Criar`, `Jogos.Editar`, `Ingressos.Visualizar`, `Ingressos.Gerenciar`; storage `OpponentLogos` (Local/Cloudinary) em `IOpponentLogoStorage`; `ITicketProvider` com `MockTicketProvider` (singleton); SPA `/admin/games`, `/admin/tickets` (lista: solicitação pendente/emitido, torcedor, plano, alternância com `Ingressos.Gerenciar`); ver `docs/architecture/parte-b8-games-tickets-admin.md`.
...
* **B.10 Loyalty & Benefits (admin):** tabelas `LoyaltyCampaigns`, `LoyaltyPointRules`, `LoyaltyPointLedgerEntries`, `BenefitPartners`, `BenefitOffers` (incl. `BannerUrl`, `IsShirtCustomizationOffer`), elegibilidades, `BenefitShirtCatalogOptions` (tamanhos/modelos por oferta) e `BenefitRedemptions` (incl. `Status` Pending/Approved/Rejected, personalização de camisa, **frete Melhor Envio ou retirada (`ShippingMethod`)** e revisão admin); API `GET/POST/PUT /api/admin/loyalty/...`, rankings, ajuste manual; `GET/POST/PUT /api/admin/benefits/...`, **`POST/DELETE /api/admin/benefits/offers/{id}/banner`**, **`PUT /api/admin/benefits/offers/{id}/shirt-catalog`**, **`POST /api/admin/benefits/redemptions/{id}/approve`**, **`POST /api/admin/benefits/redemptions/{id}/reject`**, `GET /api/admin/benefits/redemptions?status=pending|approved|rejected` (storage `BenefitOfferBanner`: Local/Cloudinary) e resgate (**bloqueado** para ofertas `IsShirtCustomizationOffer` — usar fila de aprovação); permissões `Fidelidade.Visualizar`, `Fidelidade.Gerenciar`, `Beneficios.Visualizar`, `Beneficios.Gerenciar`; gatilhos de pontos pós-conciliação de pagamento e pós-compra/resgate de ingresso (`ILoyaltyPointsTriggerPort`); SPA `/admin/loyalty`; **`/admin/benefits`** com CRUD de parceiros e ofertas, **vigência `StartAt`/`EndAt`**, oferta camisa + catálogo, fila de pedidos pendentes (aprovar/recusar), upload de banner, ativar/desativar e exclusão soft (PUT), filtros e status derivado (Vigente/Programada/Expirada/Inativa), testes Vitest em `BenefitsAdminPage.test.tsx` / `benefitsAdminHelpers.test.ts`; ver `docs/architecture/parte-b10-loyalty-benefits-admin.md`.
...
* **B.12 Integrações por token (admin):** gestão de API keys de parceiro no backoffice para consumo externo do Partner Lookup (`GET /api/partner/v1/lookup?phone=`) autenticado via `X-Api-Key`; API admin `GET/POST /api/admin/partner-keys`, `DELETE /api/admin/partner-keys/{id}` com split de autorização (`WebhooksRead` para listagem; `Webhooks.Gerenciar` para criar/revogar); permissões granulares `Webhooks.Visualizar` e `Webhooks.Gerenciar`; SPA `/admin/webhook-tokens` com abas/botões (shadcn mínimo), exibição do `plaintextKey` apenas na criação e revogação imediata; ver `docs/architecture/parte-b12-integracoes-token-admin.md`.
...
* **C.4 Jogos e ingressos (torcedor):** `GET /api/games` (jogos ativos com `GameDate` ≥ UTC agora; itens com `opponentLogoUrl` opcional), `GET /api/tickets`, `GET /api/tickets/{id}`, `POST /api/tickets/request` (sócio `Ativo`, sem duplicar par `userId`+`gameId`), `POST /api/tickets/{id}/redeem` (JWT); `IGameTorcedorReadPort`, `ITicketTorcedorPort` + queries/commands em `Modules/Torcedor`; SPA `/games` (solicitação) e `/tickets`; ver `docs/architecture/parte-c4-jogos-ingressos-torcedor.md`.
...
* **C.2 Notícias e benefícios (torcedor):** `GET /api/news`, `GET /api/news/{id}`, `GET /api/benefits/eligible`, `GET /api/benefits/offers/{offerId}` (respostas incluem `bannerUrl`, oferta camisa: `isShirtCustomizationOffer`, `shirtSizes`, `shirtModels`, `redemptionWorkflowStatus` incl. `awaiting_shipping_payment` quando frete Stripe pendente), `GET /api/benefits/shipping-options?cep=` (JWT; cotação Melhor Envio; lista vazia sem token configurado), `POST /api/benefits/offers/{offerId}/redeem` (JWT; oferta camisa exige `shippingMethod` `pickup`|`carrier`; se `carrier`: campos de camisa **e** endereço **e** opção de frete escolhida + resposta pode incluir `checkoutUrl` para pagar frete no Stripe; webhook D.4 confirma pagamento e autoaprova; se `pickup`: só camisa; ViaCEP no modo envio; persistência em `BenefitRedemptions.Shipping*` e `ShippingPaymentId`/`ShippingPaidAtUtc`); `ITorcedorNewsReadPort`, `ITorcedorBenefitsReadPort`, `ITorcedorBenefitRedemptionPort`, `IMelhorEnvioShippingPort`; CQRS em `Modules/Torcedor`; SPA `/news`, `/news/:newsId`, `/benefits`, `/benefits/:offerId`, carrossel de benefícios na home (`DashboardPage`, layout `bannerUrl`: proporção `300:148` + texto reduzido); ver `docs/architecture/parte-c2-noticias-beneficios-torcedor.md`.
