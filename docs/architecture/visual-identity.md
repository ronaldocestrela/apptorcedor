...
O **painel admin** usa avatar com iniciais no header. O **header do dashboard** (`.dash-header`) não exibe círculo de usuário. Em **Conta** (`/account`, layout Figma *perfil* / node `54:387`), a implementação no SPA espelha o mock: barra superior **`.account-page__perfil-top`** com **avatar circular 50px** (foto ou iniciais), **nome** (`.account-page__perfil-top-name`) e **configurações** (engrenagem). No **cartão de sócio** (`.account-page__perfil-card`, corpo em `.account-page__perfil-card-body`), o texto do plano (`.account-page__perfil-plan-name`) e os fatos em lista; o **quadrado de iniciais** verde em **`.account-page__perfil-initials`**; CTA **“Expandir Carteirinha”** em **`.account-page__perfil-expand`**. Atalhos “Meus benefícios” / “Fidelidade” usam **`.account-page__benefits-btn.account-page__benefits-btn--perfil`**.

**Nota de layout (desktop = mesmo desenho mobile):** o `<main>` combina `app-shell app-shell--narrow` (largura máxima do shell) com `account-page account-page--perfil`. O padding horizontal de **`.account-page`** é em **rem** (e refinado em *breakpoints* em **`.account-page--perfil`**), e **não** em `%` do viewport: um `%` da largura de **`.account-root`** (full width) com `width` do `main` limitada empurrava/“zerava” a coluna de conteúdo no desktop. A coluna alinhada ao mock (~**375px**) segue vindo de **`max-width: 23.4375rem`** e **`margin-left` / `margin-right: auto`** nos cartões e CTAs, não de padding simétrico em `%` no `main`.
