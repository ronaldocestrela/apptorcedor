...
| POST | `/api/account/register` | Anônimo | Cadastro público; retorna o mesmo contrato de sessão que o login (`AuthResponse`). O corpo **deve** incluir `phoneNumber` (celular) não vazio, após `trim` (rejeita `400` com `errors` se ausente, somente em brancos). |
...
- **Cadastro (`/register`):** checklist de requisitos de senha em tempo real (mínimo 8 caracteres, maiúscula, minúscula e dígito), alinhada às `PasswordOptions` do Identity no backend; botão **Criar conta** permanece desabilitado até todas as regras serem atendidas; **Celular** é obrigatório (validação no envio, mensagem se vazio; `aria-required` no campo). Em falha de `POST /api/account/register` com **400** e corpo `{ "errors": string[] }`, a mensagem retornada pela API é exibida no formulário (fallback genérico nos demais casos). Regras no cliente estão centralizadas em `frontend/src/features/account/registerPasswordRules.ts` para manter paridade com `AppTorcedor.Infrastructure/DependencyInjection.cs` — se a política no servidor mudar, atualizar esse módulo (ou futuramente expor regras via API).
...
- **Application:** `RegisterTorcedorCommandHandlerTests` (normalização de entrada; telefone em brancos mapeia para string vazia no port, onde o serviço rejeita); `CpfNumberTests` (CPF com/sem máscara, inválido e repetição de dígitos).
- **API:** `PartC1TorcedorAccountTests` (requisitos, cadastro, **celular obrigatório** (400 com mensagem de negócio), perfil, CPF inválido, conflito com CPF do seed, `me`, foto local, Google com fake validator); `PartC1CloudinaryProfilePhotoUploadTests` (foto com provider Cloudinary em memória: não apaga no Cloudinary quando a URL anterior e a nova partilham o mesmo `public_id`; ainda apaga quando o `public_id` antigo é distinto, ex. migração/URL alheia).
...
- **Frontend — cadastro:** `src/features/account/registerPasswordRules.test.ts` (regras de senha); `src/pages/RegisterPage.test.tsx` (checklist, desabilitar envio, submissão com senha válida, erros 400 da API); `src/features/account/accountApi.test.ts` (`formatRegisterApiErrorMessage`).
