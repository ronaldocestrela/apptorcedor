...
Rotas do **torcedor** fora do `/admin`: **C.1** — `/register` (cadastro público + LGPD), `/account` (Minha conta: perfil e foto), login com Google opcional (`VITE_GOOGLE_CLIENT_ID`) — [parte-c1-cadastro-perfil-torcedor.md](../architecture/parte-c1-cadastro-perfil-torcedor.md); **C.2** — `/news` (feed), `/news/:newsId` (detalhe), `/benefits` (benefícios elegíveis) — [parte-c2-noticias-beneficios-torcedor.md](../architecture/parte-c2-noticias-beneficios-torcedor.md); **C.3** — `/digital-card` (carteirinha digital + cache local opcional) — [parte-c3-carteirinha-torcedor.md](../architecture/parte-c3-carteirinha-torcedor.md).
