...
A SPA consome autenticação, permissões granulares, diagnóstico, configurações, auditoria, matriz role × permissão (leitura e edição), gestão de staff (convites e usuários internos), dashboard administrativo, **módulo Membership (B.4)** (listagem, detalhe, histórico operacional e alteração de status com motivo obrigatório), **módulo Plans (B.5)** (CRUD de planos, benefícios por plano, publicação para catálogo do torcedor), **módulo Payments (B.6)** (listagem de cobranças, detalhe, conciliação, cancelamento e estorno conforme permissões), **módulo Digital Card (B.7)** (listagem de emissões, preview do template fixo, emissão por lista de associações elegíveis, regeneração e invalidação conforme permissões), **módulo Games & Tickets (B.8)** (CRUD de jogos, listagem de ingressos, reserva/compra/sync via provedor mock e resgate administrativo conforme permissões), **módulo News (B.9)** (editoria de notícias, publicação/despublicação e disparo ou agendamento de notificações in-app conforme permissão), **módulos Loyalty & Benefits (B.10)** (campanhas/regras de pontos, extrato, ranking mensal/acumulado, parceiros, ofertas com **catálogo de camisa** e **fila de aprovação** de pedidos pendentes, resgates administrativos conforme permissões), **módulo Support / chamados (B.11)** (filas, SLA, histórico, respostas e gestão de status com `Chamados.Responder`; anexos de imagem com miniatura e modal de ampliação no detalhe; ver [parte-b11-support-admin.md](../architecture/parte-b11-support-admin.md)) e **módulo Integrações por token (B.12)** (listagem/geração/revogação de API key para Partner Lookup com permissões granulares `Webhooks.Visualizar` e `Webhooks.Gerenciar`).

Rotas do **torcedor** fora do `/admin`: **C.1** — `/register` (cadastro público + LGPD), `/account` (Minha conta: perfil e foto), login com Google opcional (`VITE_GOOGLE_CLIENT_ID`) — [parte-c1-cadastro-perfil-torcedor.md](../architecture/parte-c1-cadastro-perfil-torcedor.md); **C.2** — `/news` (feed), `/news/:newsId` (detalhe), `/benefits` (benefícios elegíveis; detalhe com formulário de camisa quando aplicável) — [parte-c2-noticias-beneficios-torcedor.md](../architecture/parte-c2-noticias-beneficios-torcedor.md); **C.3** — `/digital-card` (carteirinha digital + cache local opcional) — [parte-c3-carteirinha-torcedor.md](../architecture/parte-c3-carteirinha-torcedor.md); **C.4** — `/games` (jogos ativos), `/tickets` (meus ingressos e resgate) — [parte-c4-jogos-ingressos-torcedor.md](../architecture/parte-c4-jogos-ingressos-torcedor.md).
...
| `/admin/digital-cards` | `Carteirinha.Visualizar` para listar/detalhar; `Carteirinha.Gerenciar` para carregar candidatos à emissão, emitir, regenerar e invalidar (ver [parte-b7-digital-card-admin.md](../architecture/parte-b7-digital-card-admin.md)) |
...
| `/admin/benefits` | `Beneficios.Visualizar` / `Beneficios.Gerenciar`; UI inclui catálogo de tamanhos/modelos por oferta de camisa, fila de pedidos pendentes (aprovar/recusar) com **endereço de entrega** exibido, **lista de camisas já aprovadas** (personalização + entrega) (ver [parte-b10-loyalty-benefits-admin.md](../architecture/parte-b10-loyalty-benefits-admin.md)) |
| `/admin/support` | `Chamados.Responder` (ver [parte-b11-support-admin.md](../architecture/parte-b11-support-admin.md)) |
| `/admin/webhook-tokens` | `Webhooks.Visualizar` para listar; `Webhooks.Gerenciar` para gerar/revogar (ver [parte-b12-integracoes-token-admin.md](../architecture/parte-b12-integracoes-token-admin.md)) |
...
- Permissões **Webhooks** (`Webhooks.Visualizar`, `Webhooks.Gerenciar`) entram em `ADMIN_AREA_PERMISSIONS` para perfis técnicos de integração acessarem `/admin/webhook-tokens`.
