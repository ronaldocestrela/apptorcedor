...
public sealed class ConnectStripeWebhookEffectApplicator(
    ...
    ILogger<ConnectStripeWebhookEffectApplicator> logger)
    : IConnectStripeWebhookEffectApplicator
{
    public async Task ApplyAsync(string eventType, string rawJson, CancellationToken cancellationToken)
    {
        if (webhookBehavior.Value.StripeWebhookShadowMode)
        {
            logger.LogInformation(
                "Stripe Connect webhook shadow mode: skipping domain effects (type {EventType}, body length {Length}).",
                eventType,
                rawJson?.Length ?? 0);
            return;
        }
