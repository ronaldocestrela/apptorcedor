...
/// <summary>
/// Inbox idempotente para webhooks do gateway no contexto do tenant (sócio).
/// </summary>
public sealed class MemberPaymentWebhookInbox : AggregateRoot
{
    private MemberPaymentWebhookInbox()
    {
    }
    ...
    public static MemberPaymentWebhookInbox Receive(string idempotencyKey, string eventType, string rawPayload)
    {
        if (string.IsNullOrWhiteSpace(idempotencyKey))
            throw new ArgumentException("Idempotency key is required.", nameof(idempotencyKey));

        return new MemberPaymentWebhookInbox
        {
