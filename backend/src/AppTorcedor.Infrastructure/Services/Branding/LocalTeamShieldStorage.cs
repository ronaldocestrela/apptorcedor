...
public sealed class LocalTeamShieldStorage(
    ...
    IOptions<TeamShieldStorageOptions> options) : ITeamShieldStorage
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var root = string.IsNullOrWhiteSpace(options.Value.RootPath)
            ? Path.Combine(env.ContentRootPath, "wwwroot", "uploads", "team-shield")
        ...
        return $"/uploads/team-shield/{storedName}";
    }
    ...
    public Task<bool> DeleteTeamShieldAsync(string shieldUrl, CancellationToken cancellationToken = default)
    {
        ...
        const string prefix = "/uploads/team-shield/";
        if (!shieldUrl.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
            return Task.FromResult(false);

        var relative = shieldUrl[prefix.Length..].Replace('/', Path.DirectorySeparatorChar);
        var root = string.IsNullOrWhiteSpace(options.Value.RootPath)
            ? Path.Combine(env.ContentRootPath, "wwwroot", "uploads", "team-shield")
            : options.Value.RootPath;
