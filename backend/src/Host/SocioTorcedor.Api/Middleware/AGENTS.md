...
- `TenantResolutionMiddleware.cs` — ignora `/health`, `/swagger`, `/scalar` e **`/api/backoffice`**; **400** se `X-Tenant-Id` ausente nas demais rotas; **404** se tenant não encontrado
...
- `DynamicCorsMiddleware.cs` — ignora `/health`, `/swagger`, `/scalar` e `/api/backoffice`; valida `Origin` quando há `TenantContext`; `Access-Control-Allow-Headers` inclui `X-Tenant-Id` e **`X-Api-Key`**
